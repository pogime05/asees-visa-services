"use client";

import { useRef } from "react";
import Link from "next/link";
import Image from "next/image";
import { motion, useScroll, useTransform, type Variants } from "framer-motion";
import {
  ShieldCheck,
  Plane,
  BookOpen,
  Briefcase,
  Home,
  FileCheck,
  Star,
  Award,
  Zap,
  Users,
  Phone,
  MapPin,
  ChevronRight,
  ArrowRight,
} from "lucide-react";
import FloatingOrbs from "@/components/FloatingOrbs";
import Card3D from "@/components/Card3D";
import CountUp from "@/components/CountUp";
import TestimonialsMarquee from "@/components/TestimonialsMarquee";

// ── Data ─────────────────────────────────────────────────────────────────────

const services = [
  {
    icon: ShieldCheck,
    title: "Passport Services",
    desc: "New applications, renewals, and urgent processing for Canadian passports.",
    color: "from-cyan-400 to-cyan-600",
    border: "border-cyan-500/20",
  },
  {
    icon: Plane,
    title: "Visitor Visas",
    desc: "Tourist and family visit visas with expert guidance on documentation.",
    color: "from-blue-400 to-blue-600",
    border: "border-blue-500/20",
  },
  {
    icon: BookOpen,
    title: "Study Permits",
    desc: "Student visa applications and permit extensions for international students.",
    color: "from-violet-400 to-violet-600",
    border: "border-violet-500/20",
  },
  {
    icon: Briefcase,
    title: "Work Permits",
    desc: "Open and employer-specific work permits for skilled workers.",
    color: "from-indigo-400 to-indigo-600",
    border: "border-indigo-500/20",
  },
  {
    icon: Home,
    title: "Permanent Residency",
    desc: "Express Entry, Provincial Nominee, and family sponsorship applications.",
    color: "from-teal-400 to-teal-600",
    border: "border-teal-500/20",
  },
  {
    icon: FileCheck,
    title: "Document Authentication",
    desc: "Notarization, apostille, and document legalization services.",
    color: "from-sky-400 to-sky-600",
    border: "border-sky-500/20",
  },
];

const reasons = [
  {
    icon: Star,
    title: "5-Star Rated",
    desc: "Consistently rated 5 stars by our clients on Google with over 500 reviews.",
    color: "from-amber-400 to-yellow-500",
  },
  {
    icon: Award,
    title: "15+ Years Experience",
    desc: "Over a decade of proven expertise in Canadian immigration and visa services.",
    color: "from-cyan-400 to-cyan-600",
  },
  {
    icon: Zap,
    title: "Fast Turnaround",
    desc: "We process applications quickly without compromising accuracy or quality.",
    color: "from-blue-400 to-indigo-500",
  },
  {
    icon: Users,
    title: "Personalized Support",
    desc: "One-on-one guidance from start to finish — we're with you every step of the way.",
    color: "from-violet-400 to-purple-500",
  },
];

const heroStats: Array<{
  to: number;
  decimals?: number;
  suffix: string;
  label: string;
  sub: string;
  color: string;
}> = [
  { to: 5, decimals: 1, suffix: "★", label: "Google Rating", sub: "500+ reviews", color: "text-amber-400" },
  { to: 10000, suffix: "+", label: "Clients Served", sub: "Since 2009", color: "text-cyan-400" },
  { to: 15, suffix: "+", label: "Years in Business", sub: "Brampton's trusted choice", color: "text-violet-400" },
  { to: 20, suffix: "+", label: "Services Offered", sub: "All immigration needs", color: "text-blue-400" },
];

// ── Animation variants ────────────────────────────────────────────────────────

const container: Variants = {
  hidden: {},
  visible: { transition: { staggerChildren: 0.1 } },
};

const item: Variants = {
  hidden: { opacity: 0, y: 28 },
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.65, ease: "easeOut" },
  },
};

const cardItem: Variants = {
  hidden: { opacity: 0, y: 40 },
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.55, ease: "easeOut" },
  },
};

const staggerGrid: Variants = {
  hidden: {},
  visible: { transition: { staggerChildren: 0.08 } },
};

// ── Page ──────────────────────────────────────────────────────────────────────

export default function HomePage() {
  const heroRef = useRef<HTMLElement>(null);
  const { scrollYProgress } = useScroll({
    target: heroRef,
    offset: ["start start", "end start"],
  });
  const heroY = useTransform(scrollYProgress, [0, 1], [0, 100]);

  return (
    <>
      {/* ════════════════════════════════ HERO ════════════════════════════════ */}
      <section
        ref={heroRef}
        className="relative min-h-screen flex items-center mesh-bg text-white overflow-hidden"
      >
        <FloatingOrbs />
        <div className="absolute inset-0 pattern-dots-dark opacity-40" />
        <div
          className="absolute inset-0 opacity-[0.03]"
          style={{
            backgroundImage:
              "linear-gradient(to right,white 1px,transparent 1px),linear-gradient(to bottom,white 1px,transparent 1px)",
            backgroundSize: "80px 80px",
          }}
        />

        {/* Decorative concentric rings */}
        <motion.div
          className="absolute top-1/2 right-[6%] -translate-y-1/2 hidden xl:block pointer-events-none"
          style={{ y: heroY }}
          initial={{ opacity: 0, scale: 0.75 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 1.4, delay: 0.6, ease: [0.22, 1, 0.36, 1] }}
        >
          <div className="relative w-[440px] h-[440px]">
            <div className="absolute inset-0 rounded-full border border-cyan-500/10" />
            <div className="absolute inset-[15%] rounded-full border border-cyan-500/10" />
            <div className="absolute inset-[30%] rounded-full border border-cyan-400/15" />
            <div className="absolute inset-[42%] rounded-full bg-cyan-500/8 blur-xl" />
          </div>
        </motion.div>

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 w-full">
          <div className="grid lg:grid-cols-5 gap-12 items-center">
            {/* ── Left: staggered copy ── */}
            <motion.div
              className="lg:col-span-3"
              variants={container}
              initial="hidden"
              animate="visible"
            >
              {/* Badge */}
              <motion.div variants={item}>
                <div className="inline-flex items-center gap-2 glass-card px-4 py-2 rounded-full text-sm font-medium mb-7">
                  <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
                  <span className="text-slate-300">5.0 Google Rating</span>
                  <span className="w-px h-3.5 bg-white/20" />
                  <span className="text-cyan-400 font-semibold">Brampton, Ontario</span>
                </div>
              </motion.div>

              {/* Headline */}
              <motion.h1
                variants={item}
                className="text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight mb-6"
              >
                Your Trusted
                <br />
                <span className="gradient-text">Passport &amp; Visa</span>
                <br />
                Experts
              </motion.h1>

              {/* Sub-copy */}
              <motion.p
                variants={item}
                className="text-lg text-slate-400 leading-relaxed mb-8 max-w-xl"
              >
                From visitor visas to permanent residency — Asees Passport and Visa Services
                handles every step of your immigration journey with care, speed, and expertise.
              </motion.p>

              {/* CTAs */}
              <motion.div
                variants={item}
                className="flex flex-col sm:flex-row gap-3 mb-8"
              >
                <a
                  href="tel:9056161739"
                  className="group inline-flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-400 hover:to-cyan-500 text-white font-bold px-8 py-4 rounded-full glow-cyan-sm hover:glow-cyan transition-all duration-300 shadow-lg"
                >
                  <Phone className="w-4 h-4" />
                  Call Now: (905) 616-1739
                  <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </a>
                <Link
                  href="/services"
                  className="inline-flex items-center justify-center gap-2 glass-card hover:bg-white/10 text-white font-semibold px-8 py-4 rounded-full transition-all duration-300"
                >
                  View All Services
                  <ChevronRight className="w-4 h-4" />
                </Link>
              </motion.div>

              {/* Trust micro-copy */}
              <motion.div variants={item} className="flex flex-wrap gap-5">
                {["No Hidden Fees", "Same-Day Consultations", "Multilingual Staff"].map(
                  (tag) => (
                    <span key={tag} className="flex items-center gap-1.5 text-xs text-slate-400">
                      <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 shrink-0" />
                      {tag}
                    </span>
                  )
                )}
              </motion.div>
            </motion.div>

            {/* ── Right: floating stats card ── */}
            <div className="lg:col-span-2 hidden lg:block">
              <motion.div
                initial={{ opacity: 0, x: 50, y: 16 }}
                animate={{ opacity: 1, x: 0, y: 0 }}
                transition={{ duration: 0.9, delay: 0.45, ease: [0.22, 1, 0.36, 1] }}
                className="gradient-border animate-float"
              >
                <div className="glass-card rounded-2xl p-6 space-y-2">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-5">
                    Why Clients Trust Us
                  </p>
                  {heroStats.map((s) => (
                    <div
                      key={s.label}
                      className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/5 transition-colors"
                    >
                      <CountUp
                        to={s.to}
                        suffix={s.suffix}
                        decimals={s.decimals}
                        className={`text-2xl font-black w-20 shrink-0 tabular-nums ${s.color}`}
                      />
                      <div>
                        <p className="text-white font-semibold text-sm">{s.label}</p>
                        <p className="text-slate-500 text-xs">{s.sub}</p>
                      </div>
                    </div>
                  ))}
                </div>
              </motion.div>
            </div>
          </div>
        </div>

        <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-slate-950/80 to-transparent" />
      </section>

      {/* ══════════════════════════════ SERVICES ══════════════════════════════ */}
      <section className="relative py-24 overflow-hidden bg-[#030712]">
        {/* Toronto skyline bg */}
        <div className="absolute inset-0">
          <Image
            src="https://images.unsplash.com/photo-1507992781348-310259076fe0?w=1920&auto=format&fit=crop&q=80"
            alt="Toronto skyline"
            fill
            className="object-cover object-center opacity-20"
            priority
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[#030712]/60 via-[#030712]/70 to-[#030712]" />
        </div>
        <div className="absolute inset-0 pattern-dots-dark opacity-30" />

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Section header */}
          <motion.div
            className="text-center mb-14"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: "-80px" }}
            variants={container}
          >
            <motion.span
              variants={item}
              className="inline-flex items-center gap-2 text-cyan-400 text-sm font-semibold uppercase tracking-widest mb-3"
            >
              <span className="h-px w-8 bg-cyan-400" /> What We Offer{" "}
              <span className="h-px w-8 bg-cyan-400" />
            </motion.span>
            <motion.h2
              variants={item}
              className="text-3xl md:text-5xl font-black text-white mb-4"
            >
              What We Can Do For You
            </motion.h2>
            <motion.p variants={item} className="text-slate-400 max-w-xl mx-auto text-lg">
              Comprehensive immigration and documentation services under one roof.
            </motion.p>
          </motion.div>

          {/* Services grid with stagger */}
          <motion.div
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: "-60px" }}
            variants={staggerGrid}
          >
            {services.map((s) => {
              const Icon = s.icon;
              return (
                <motion.div key={s.title} variants={cardItem}>
                  <Card3D className="h-full">
                    <div
                      className={`h-full glass-card rounded-2xl p-6 border ${s.border} hover:border-opacity-60 transition-all duration-300 group`}
                    >
                      <div
                        className={`w-12 h-12 rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center mb-5 shadow-lg group-hover:scale-110 transition-transform duration-300`}
                      >
                        <Icon className="w-6 h-6 text-white" />
                      </div>
                      <h3 className="font-bold text-white text-lg mb-2 group-hover:text-cyan-300 transition-colors">
                        {s.title}
                      </h3>
                      <p className="text-slate-400 text-sm leading-relaxed">{s.desc}</p>
                      <div className="mt-4 flex items-center gap-1 text-cyan-500 text-xs font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
                        Learn more <ChevronRight className="w-3 h-3" />
                      </div>
                    </div>
                  </Card3D>
                </motion.div>
              );
            })}
          </motion.div>

          <motion.div
            className="text-center mt-12"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
          >
            <Link
              href="/services"
              className="inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold px-8 py-4 rounded-full glow-cyan-sm hover:glow-cyan transition-all duration-300 shadow-lg"
            >
              Explore All Services <ArrowRight className="w-4 h-4" />
            </Link>
          </motion.div>
        </div>
      </section>

      {/* ══════════════════════════ WHY ASEES + STATS ══════════════════════════ */}
      <section className="relative py-24 bg-slate-950 overflow-hidden">
        <FloatingOrbs />
        <div className="absolute inset-0 pattern-dots-dark opacity-20" />

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            {/* Left: reasons */}
            <div>
              <motion.div
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true, margin: "-60px" }}
                variants={container}
              >
                <motion.span
                  variants={item}
                  className="inline-flex items-center gap-2 text-cyan-400 text-sm font-semibold uppercase tracking-widest mb-4"
                >
                  <span className="h-px w-8 bg-cyan-400" /> Why Asees
                </motion.span>
                <motion.h2
                  variants={item}
                  className="text-4xl md:text-5xl font-black text-white mb-6 leading-tight"
                >
                  Why Thousands
                  <br />
                  <span className="gradient-text">Choose Asees</span>
                </motion.h2>
                <motion.p variants={item} className="text-slate-400 mb-10 leading-relaxed text-lg">
                  Located in Brampton at 30 Intermodal Drive, we&apos;ve built our reputation on
                  trust, accuracy, and results. Our clients come back — and they bring their
                  families.
                </motion.p>
              </motion.div>

              <motion.div
                className="space-y-4"
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true, margin: "-60px" }}
                variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.1 } } }}
              >
                {reasons.map((r) => {
                  const Icon = r.icon;
                  return (
                    <motion.div key={r.title} variants={cardItem}>
                      <Card3D>
                        <div className="glass-card rounded-xl p-4 flex items-start gap-4 hover:border-cyan-500/30 transition-colors border border-white/10">
                          <div
                            className={`w-10 h-10 rounded-xl bg-gradient-to-br ${r.color} flex items-center justify-center shrink-0 shadow-lg`}
                          >
                            <Icon className="w-5 h-5 text-white" />
                          </div>
                          <div>
                            <h4 className="font-bold text-white text-sm">{r.title}</h4>
                            <p className="text-slate-400 text-xs mt-0.5 leading-relaxed">
                              {r.desc}
                            </p>
                          </div>
                        </div>
                      </Card3D>
                    </motion.div>
                  );
                })}
              </motion.div>
            </div>

            {/* Right: animated stat tiles */}
            <motion.div
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true, margin: "-60px" }}
              variants={container}
            >
              <motion.p variants={item} className="text-slate-400 text-sm font-medium mb-5">
                By the numbers
              </motion.p>

              <motion.div variants={staggerGrid} className="grid grid-cols-2 gap-4 mb-4">
                {[
                  {
                    to: 500,
                    suffix: "+",
                    label: "5-Star Reviews",
                    color: "text-amber-400",
                    bg: "from-amber-500/10 to-yellow-500/5",
                    border: "border-amber-500/20",
                  },
                  {
                    to: 10000,
                    suffix: "+",
                    label: "Clients Served",
                    color: "text-cyan-400",
                    bg: "from-cyan-500/10 to-cyan-400/5",
                    border: "border-cyan-500/20",
                  },
                  {
                    to: 15,
                    suffix: "+",
                    label: "Years Experience",
                    color: "text-violet-400",
                    bg: "from-violet-500/10 to-violet-400/5",
                    border: "border-violet-500/20",
                  },
                  {
                    to: 20,
                    suffix: "+",
                    label: "Services Offered",
                    color: "text-blue-400",
                    bg: "from-blue-500/10 to-blue-400/5",
                    border: "border-blue-500/20",
                  },
                ].map((stat) => (
                  <motion.div
                    key={stat.label}
                    variants={cardItem}
                    className={`glass-card rounded-2xl p-5 border ${stat.border} bg-gradient-to-br ${stat.bg}`}
                  >
                    <CountUp
                      to={stat.to}
                      suffix={stat.suffix}
                      className={`text-3xl font-black tabular-nums ${stat.color}`}
                    />
                    <p className="text-slate-400 text-xs mt-1">{stat.label}</p>
                  </motion.div>
                ))}
              </motion.div>

              {/* Google rating card */}
              <motion.div
                variants={item}
                className="glass-card rounded-2xl p-5 border border-white/10"
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className="w-4 h-4 text-amber-400 fill-amber-400" />
                    ))}
                  </div>
                  <span className="text-white font-bold text-sm">5.0</span>
                  <span className="text-slate-500 text-xs">on Google</span>
                </div>
                <p className="text-slate-400 text-sm leading-relaxed">
                  Consistently rated 5 stars across 500+ verified Google reviews. Brampton&apos;s
                  most trusted passport &amp; visa office.
                </p>
              </motion.div>
            </motion.div>
          </div>
        </div>

        {/* ── Testimonials marquee ── */}
        <div className="mt-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
            >
              <span className="inline-flex items-center gap-2 text-cyan-400 text-sm font-semibold uppercase tracking-widest">
                <span className="h-px w-8 bg-cyan-400" /> What Our Clients Say
              </span>
              <h3 className="text-2xl font-black text-white mt-2">Real Stories, Real Results</h3>
            </motion.div>
          </div>
          <TestimonialsMarquee />
        </div>
      </section>

      {/* ═══════════════════════════════ STATS BAR ════════════════════════════ */}
      <section className="bg-gradient-to-r from-cyan-950 via-blue-950 to-indigo-950 border-y border-cyan-500/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <motion.div
            className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: "-60px" }}
            variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.1 } } }}
          >
            {[
              { to: 5, decimals: 1, suffix: "★", label: "Google Rating", color: "text-amber-400" },
              { to: 500, suffix: "+", label: "5-Star Reviews", color: "text-cyan-400" },
              { to: 15, suffix: "+", label: "Years in Business", color: "text-blue-400" },
              { to: 10000, suffix: "+", label: "Clients Served", color: "text-violet-400" },
            ].map((s) => (
              <motion.div key={s.label} variants={cardItem}>
                <CountUp
                  to={s.to}
                  suffix={s.suffix}
                  decimals={s.decimals}
                  className={`text-3xl md:text-4xl font-black tabular-nums ${s.color}`}
                />
                <p className="text-sm text-slate-400 mt-1">{s.label}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      </section>

      {/* ═══════════════════════════════════ CTA ══════════════════════════════ */}
      <section className="relative bg-[#030712] py-24 overflow-hidden">
        <FloatingOrbs />
        <div className="absolute inset-0 pattern-dots-dark opacity-30" />
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-[500px] h-[300px] bg-cyan-500/5 rounded-full blur-3xl" />
        </div>

        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: "-60px" }}
            variants={container}
          >
            <motion.div variants={item}>
              <div className="inline-flex items-center gap-2 glass-card px-4 py-2 rounded-full text-sm font-medium mb-6 border border-cyan-500/20">
                <span className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse" />
                <span className="text-slate-300">Now accepting new clients</span>
              </div>
            </motion.div>

            <motion.h2
              variants={item}
              className="text-4xl md:text-6xl font-black text-white mb-5 leading-tight"
            >
              Ready to Start
              <br />
              <span className="gradient-text">Your Journey?</span>
            </motion.h2>

            <motion.p variants={item} className="text-slate-400 mb-10 text-lg max-w-xl mx-auto">
              Visit us in Brampton or give us a call. Our experts are ready to help you take the
              next step.
            </motion.p>

            <motion.div
              variants={item}
              className="flex flex-col sm:flex-row gap-4 justify-center"
            >
              <a
                href="tel:9056161739"
                className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-400 hover:to-cyan-500 text-white font-bold px-9 py-4 rounded-full glow-cyan transition-all duration-300 shadow-xl shadow-cyan-500/20"
              >
                <Phone className="w-4 h-4" />
                Call (905) 616-1739
              </a>
              <Link
                href="/contact"
                className="inline-flex items-center justify-center gap-2 glass-card hover:bg-white/10 border border-white/10 text-white font-semibold px-9 py-4 rounded-full transition-all duration-300"
              >
                <MapPin className="w-4 h-4 text-cyan-400" />
                Visit Brampton Office
              </Link>
            </motion.div>
          </motion.div>
        </div>
      </section>
    </>
  );
}
